...
class ISICLightningModule(LightningModule):
    ...
    def on_validation_epoch_end(self):
        ...
        self.log("val_weighted_f1", f1_weighted)
        self.log("val_auroc", auroc)

        # Log per-class metrics
        for i in range(self.num_classes):
            self.log(f"val_precision_class_{i}", prec[i])
            self.log(f"val_recall_class_{i}", rec[i])
            self.log(f"val_f1_class_{i}", f1_pc[i])
    ...
    def configure_optimizers(self):
        ...
        groups = []
        groups.append({"params": backbone.layer1.parameters(), "lr": lr * 0.1})
        groups.append({"params": backbone.layer2.parameters(), "lr": lr * 0.3})
        groups.append({"params": backbone.layer3.parameters(), "lr": lr * 0.6})
        groups.append({"params": backbone.layer4.parameters(), "lr": lr * 1.0})
        groups.append({"params": self.model.meta_mlp.parameters(), "lr": lr * 3.0})
        groups.append({"params": self.model.classifier.parameters(), "lr": lr * 3.0})
    ...
    def on_train_epoch_start(self):
        if self.current_epoch < self.warmup_epochs:
            if not self.freeze_backbone:
                self.freeze_backbone = True
                self._apply_backbone_freeze()
                print(f"[Epoch {self.current_epoch}] Backbone → FROZEN (warmup)")
        else:
            if self.freeze_backbone:
                self.freeze_backbone = False
                self._apply_backbone_unfreeze()
                print(f"[Epoch {self.current_epoch}] Backbone → UNFROZEN (fine-tuning)")
